    ...
    if not st.session_state.logged_in:
        ...
        with t_log:
            if not st.session_state.forgot_pw:
                # .strip() verhindert Leerzeichen-Fehler am Handy
                u_input = st.text_input("Nutzername", key="l_u").strip()
                p_input = st.text_input("Passwort", type="password", key="l_p").strip()
                if st.button("Anmelden"):
                    df_n = hole_df("nutzer")
                    if not df_n.empty:
                        # Case-Insensitive Vergleich (optional)
                        user_match = df_n[(df_n['benutzername'] == u_input) & (df_n['passwort'] == hash_passwort(p_input))]
                ...
                res_u = st.text_input("Nutzername").strip()
                res_m = st.text_input("E-Mail").strip()
                neu_p = st.text_input("Neues Passwort", type="password").strip()
                if st.button("Passwort ändern"):
                    if check_user_mail_match(res_u, res_m) and update_passwort(res_u, neu_p):
                        st.success("Erfolg! Bitte einloggen."); st.session_state.forgot_pw = False; st.rerun()
                    else: st.error("Daten falsch.")
        ...
        with t_reg:
            ...
            if st.button("Erstellen"):
                if nu and npw and ne and agb:
                    ...
                    else: st.error("Fehler: Name/Mail vergeben.")
                else: st.warning("Pflichtfelder + AGB!")
    else:
...
if st.session_state.wahl == "👤 Profil":
    ...
    with col_p1:
        ...
        emo = st.selectbox("Avatar Emoji", ["🧗", "🏃", "🧗‍♂️", "🤸", "🦁", "🚀", "🌟"])
    with col_p2:
        with st.form("edit_profil"):
            ...
            alt = st.number_input("Alter", value=int(user_data['alter_jahre']))
            if st.form_submit_button("Speichern"):
                if aktualisiere_profil(st.session_state.user, e_mail, v_name, n_name, alt, emo):
...
# --- HAUPTBEREICH: SUCHE ---
    ...
    if st.session_state.logged_in:
        ...
        with c_f1:
            with st.expander("💡 Spielplatz vorschlagen"):
...
# --- ADMIN BEREICH ---
    ...
    t1, t2, t3, t4 = st.tabs(["📥 Vorschläge", "👥 Nutzer", "💬 Feedback", "🏗️ Neu"])
    with t1:
